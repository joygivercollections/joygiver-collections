...
export function MobileMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="mobile-menu-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <aside ref={panelRef} className="mobile-menu" role="dialog" aria-modal="true" aria-label="Menu">
      ...
      <nav aria-label="Information"><NavLink to="/wholesale" onClick={onClose}>Wholesale <span aria-hidden="true">→</span></NavLink><NavLink to="/about" onClick={onClose}>About Us <span aria-hidden="true">→</span></NavLink><NavLink to="/contact" onClick={onClose}>Contact <span aria-hidden="true">→</span></NavLink></nav>
